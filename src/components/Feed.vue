<template>
  <v-app class="px-lg-16 px-2">
          ...
          <q-item>
            <q-item-section class="text-grey">
              ...
              <v-col cols="12">
                ...
                <VueDatePicker :enable-time-picker="false" class="mt-2" input-class-name="dp-custom-input"
                  placeholder="Enter Buy Date..." v-model="buyDate"></VueDatePicker>
              </v-col>
              <v-col cols="12">
                <span>Sell Date</span>
                <VueDatePicker class="mt-2" input-class-name="dp-custom-input" placeholder="Enter Sell Date..."
                  v-model="sellDate"></VueDatePicker>
              </v-col>
              <v-col cols="12">
                <span>Quantity <span class="text-red font-weight-bold">*</span></span>
                <v-text-field class="mt-2" @keypress="filter(event)" bg-color="white" label="Enter Quantity..."
      ...
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              <v-col cols="12">
                <v-text-field label="Account Name" v-model="accountName" required></v-text-field>
              </v-col>
              <v-col cols="12">
                <v-select :items="['USD', 'SGR']" v-model="selectedCurrency" label="Select Currency" required></v-select>
              </v-col>
            </v-row>
          </v-container>
        </v-card-text>
        <v-card-actions class="mb-2">
          <v-spacer></v-spacer>
          <v-btn style="background-color: #377DFF; color: white" @click="addAccountDialog = false">
            + Add Account
          </v-btn>
          <v-btn color="blue-darken-1" variant="text" @click="clearFields('new-account')">
            Close
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <div class="py-4">
      <v-row no-gutters="true" align="center">
        ...
        <v-col class="d-flex justify-end">
          <v-btn @click="addHoldingDialog = true" class="py-6 d-flex align-center"
    ...
    <v-card>
      <v-card-title>
        <v-spacer></v-spacer>
        <v-text-field v-model="search" append-icon="mdi-magnify" label="Search Holdings" single-line
